...
class MovingVoterProfile(Profile):
    ...
    0
    >>> moving_profile.embeddings.positions
    array([[1., 0., 0.],
           [0., 0., 1.],
           [0., 1., 0.],
           [1., 0., 0.]])
    ...
    array([[0.8, 0.8, 0.8, 0.5],
           ...
           [1. , 0.1, 0.1, 0.5]])
    """
    def __init__(self, ratings=None, embeddings=None, moving_voter=0):
        self.rule_ = None
        if ratings is None:
            ratings = np.array([[.8, .8, .8, .5], [.1, .1, 1, .5], [.1, 1, .1, .5], [1, .1, .1, .5]])
        if embeddings is None:
            embs = np.array([[1., 0., 0.], [0., 0., 1.], [0., 1., 0.], [1., 0., 0.]])
    ...
    def plot_scores_evolution(self, show=True):
        ...
        tab_y = []
        for x in tab_x:
            self.embeddings.positions[self.moving_voter] = normalize([1-x, x, 0])
